...
> If you are using a TV as your display, and find that the mouse position is strangely offset from the "true" position on the screen, open the file `config.txt` in the top level of the first partition (editable on any PC, or as `/boot/config.txt` when running the image), uncomment the `disable_overscan=1` line, and then reboot.
